...
The table provides a summary of total infant deaths by state, with each row representing a specific location. The 'location_desc' column denotes the state or city, and the 'total_infant_deaths' column indicates the corresponding number of infant deaths in each location. The data suggests variability in infant mortality rates across different regions, with some areas reporting higher or lower rates than others. For instance, states like Pennsylvania have a notably lower count of infant deaths, while others, such as Alaska and Arkansas, have higher counts. This summary provides an overview of the distribution of infant deaths across various geographical locations. 

```{r}
infant_deaths_race <- cleaned_infant_mortality |>
  filter(break_out_category == "Maternal Race/Ethnicity" & break_out == "Other non-Hispanic")|>
  filter(question == "Indicator of infant currently alive" & response == "NO")|>
  group_by(break_out_category) |>
  summarize(break_out_response = n()) |>
  kable()

print(infant_deaths_race)
```
